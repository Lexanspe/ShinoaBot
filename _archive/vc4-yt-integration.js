...
${songs.get(`${interaction.guild.id}-${queue.get(interaction.guild.id) + 2}`) && !songs.get(`${interaction.guild.id}-${queue.get(interaction.guild.id) - 1}`) ? `${queue.get(interaction.guild.id) + 2}- ${songs.get(`${interaction.guild.id}-${queue.get(interaction.guild.id) + 2}`)}` : ''}
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                for (const ext of exts) {
                    const p = `media/ytsongs/${videoId}.${ext}`;
                    if (fs.existsSync(p)) {
                        existingFile = p;
                        break;
                    }
    ...
    if (!channel) {
        interaction.reply({ content: 'You must be in a voice channel to use this command.', flags: 64 });
        return;
    } else if (channel !== guildChannels.get(interaction.guild.id) && guildConnections.has(interaction.guild.id)) {
        interaction.reply({ content: 'You must be in the same voice channel with me to use this command.', flags: 64 });
        return;
    }
...
${songs.get(`${interaction.guild.id}-${queue.get(interaction.guild.id) + 2}`) && !songs.get(`${interaction.guild.id}-${queue.get(interaction.guild.id) - 1}`) ? `${queue.get(interaction.guild.id) + 2}- ${songs.get(`${interaction.guild.id}-${queue.get(interaction.guild.id) + 2}`)}` : ''}
    ...
    collector.on('end', (collected, reason) => {
        if (reason === 'time') {
            ...
            interaction.followUp({ content: 'Collector timed out. You can call the command again by using /playyt', flags: 64 });
        }
        activeCollectors.delete(interaction.guild.id);
    ...
    collector.on('collect', async (buttonInteraction) => {
        if (!buttonInteraction.member.voice.channel) return buttonInteraction.reply({ content: 'You need to be in a voice channel to use this command.', flags: 64 });
        if (buttonInteraction.member.voice.channel !== guildChannels.get(interaction.guild.id) && guildConnections.has(interaction.guild.id)) return buttonInteraction.reply({ content: 'You must be in the same voice channel with me to use this command.', flags: 64 });  
    ...
    const modalHandler = async (modalInteraction) => {
        ...
        // Modal'ı kapat - Discord'a response gönder
            modalInteraction.deferReply({ flags: 64, fetchReply: true });
    ...
    setTimeout(async () => {
        try {
            ...
            for (const ext of exts) {
                const filePath = `media/ytsongs/${videoId}.${ext}`;
