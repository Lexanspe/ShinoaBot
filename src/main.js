...
const PRINT_TOKENS = true; // Tokenları ve API anahtarlarını konsola yazdırmak için bu değeri true yapın

if (PRINT_TOKENS) {
    console.log('Bot Token:', process.env.TOKEN);
    console.log('OpenRoute Token:', process.env.OTOKEN);
}
...
client.on(Events.InteractionCreate, async interaction => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        // For chess, we use customId prefixes like 'chess_move', 'chess_modal'
        if (interaction.customId.startsWith('chess_')) {
            commandName = 'chess';
        } else {
            return; // Unhandled button/modal
        }
    } else {
        return;
    }
    ...
    if (timestamps.has(interaction.user.id)) {
        ...
        if (now < expirationTime) {
            ...
            return interaction.reply({ content: `\`${command.data.name}\` komutunu çok hızlı kullanıyorsunuz. <t:${expiredTimestamp}:R> yeniden kullanabilirsiniz.`, flags: 64 });
        }
    }
    ...
    try {
        ...
        if (developerModule.developmentMode && interaction.user.id !== process.env.OWNERID && interaction.user.id !== process.env.DEV) {
            ...
            return interaction.reply({ content: 'Bot şu anda geliştirici modunda. Sadece geliştiriciler komut kullanabilir.', flags: 64 });
        }
        ...
        if (interaction.isChatInputCommand()) {
            ...
            if (command && command.handleModal) await command.handleModal(interaction, client);
        }
