...
async execute(interaction) {
    ...
    if (!targetMember) {
        await interaction.reply({ content: 'Bu kullanıcı sunucuda bulunamadı.', ephemeral: true });
        return;
    }
    ...
    if (targetUser.id === interaction.guild.ownerId) {
        await interaction.reply({ content: 'Sunucu sahibini atamazsın!', ephemeral: true });
        return;
    }
    ...
    try {
        ...
        await interaction.reply({ 
            content: `${targetUser.tag} başarıyla sunucudan atıldı.\n**Sebep:** ${reason}\nKuralları gözden geçirmek için: https://shinoa.xyz/kurallar`, 
        });
    } catch (error) {
