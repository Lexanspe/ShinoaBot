...
class GuildSession {
    ...
    setupPlayerListeners() {
        ...
        this.audioPlayer.on('error', error => {
            ...
            if (error.message.includes("opus") || error.message.includes("decoder") || error.message.includes("prism-media")) {
                console.error("❌ Codec/Decoder error occurred during playback. Ensure opusscript or @discordjs/opus is working properly.");
            }
    ...
    async playCurrentSong() {
        ...
        // Find file
        const ytsongsPath = path.join(__dirname, '../../media/ytsongs');
        ...
        if (filePath) {
            // Only wait for Ready if connection is not already established
            if (this.voiceConnection && this.voiceConnection.state.status !== VoiceConnectionStatus.Ready) {
                try {
                    console.log(`[Voice] Connection not Ready yet, waiting (up to 20s)...`);
                    await entersState(this.voiceConnection, VoiceConnectionStatus.Ready, 20_000);
                    console.log(`[Voice] VoiceConnection is Ready!`);
                } catch (readyError) {
                    console.error(`[Voice] Connection did NOT become Ready within 20 seconds. Aborting playback.`, readyError.message);
                    // Destroy the stale connection so it doesn't linger
                    try {
                        this.voiceConnection.destroy();
                    } catch (_) { /* ignore destroy errors */ }
                    this.voiceConnection = null;
                    // Do NOT call player.play() — just stop here cleanly
                    return;
                }
            }
            ...
            try {
                ...
                console.error(`[Audio Play Error] Failed to play ${videoId}:`, playError);
                if (playError.message.includes("Cannot find module") && (playError.message.includes("opus") || playError.message.includes("prism-media"))) {
                    console.error("\n❌ CRITICAL CODEC ERROR: An Opus encoder/decoder library is missing (e.g. opusscript or @discordjs/opus).");
                    console.error("Please run: npm install opusscript\n");
                }
                // Skip to next song on error to prevent freezing
                this.queueIndex++;
                await this.playCurrentSong();
            }
        } else {
    ...
    createV2Payload() {
        ...
        const statusEmoji = playerStatus === AudioPlayerStatus.Playing ? '▶️' :
                            playerStatus === AudioPlayerStatus.Paused ? '⏸️' : '⏹️';
            ...
            components: [
                {
                    ...
                    content: currentSong
                        ...
                        : `### 🎵 Nothing Playing\n*Press **Add Song** to queue up a track!*`
                }
            ],
        ...
        if (this.songs.size > 1) {
            ...
            for (let offset = 1; offset <= 4; offset++) {
                ...
                if (song) {
                    ...
                    queueItems.push(`${offset === 1 ? '⏭️ ' : '　　'}**${displayNumber}.** ${truncated}`);
                }
            }
        ...
        if (currentSong) {
            ...
            footerText = `🎵 ${displayIndex} / ${displayTotal}  •  🔂 ${this.isLooping ? 'On' : 'Off'}  •  🔁 ${this.isListLooping ? 'On' : 'Off'}  •  🔀 ${this.isShuffling ? 'On' : 'Off'}`;
        } else {
            footerText = `Queue is empty — add a song to begin`;
        }
        ...
        const loopSection = {
            ...
            components: [
                {
                    type: ComponentType_TextDisplay,
                    content: `🔂 **Repeat:** ${this.isLooping ? '✅ This song' : '❌ Off'}`
                }
            ],
            accessory: new ButtonBuilder()
                .setCustomId('toggle_loop')
                .setEmoji(this.isLooping ? '🔂' : '➡️')
                .setLabel(this.isLooping ? 'Stop' : 'Repeat')
                .setStyle(this.isLooping ? ButtonStyle.Danger : ButtonStyle.Secondary)
                .toJSON()
        };
        ...
        if (this.songs.size > 0) {
            secondaryRow.addComponents(
                ...
                new ButtonBuilder()
                    ...
                    .setEmoji(this.isEditing ? '🔒' : '⚙️')
                    .setLabel(this.isEditing ? 'Close' : 'Settings')
        ...
        // Root Container — accent changes with player state
        const accentColor = playerStatus === AudioPlayerStatus.Playing ? 0x5865F2 :
                            playerStatus === AudioPlayerStatus.Paused ? 0xFEE75C :
                            0x36393F;
        const rootContainer = {
...
function isPlaylistUrl(url) {
    ...
    try {
        return new URL(normalized).searchParams.has('list');
    } catch {
 ...
 * - https://youtu.be/ID
 * - youtu.be/ID  (no scheme)
 * - https://www.youtube.com/watch?v=ID
 * - youtube.com/watch?v=ID  (no scheme / no www)
...
function getYouTubeId(url) {
    try {
        ...
        // Bare 11-char video ID (alphanumeric, dash, underscore)
...
const ytDlpBinName = process.platform === 'win32' ? 'yt-dlp.exe' : 'yt-dlp';
const ytDlpPath = path.join(__dirname, '../bin', ytDlpBinName);
global.isYtDlpUpdated = global.isYtDlpUpdated || false;

async function ensureYtDlpUpdated() {
    if (global.isYtDlpUpdated) return;
    try {
        console.log("Auto-updating yt-dlp binary from GitHub...");
        ...
        if (process.platform !== 'win32') {
            await fsPromises.chmod(ytDlpPath, '755');
        }
        console.log("yt-dlp binary successfully updated.");
        global.isYtDlpUpdated = true;
    } catch (updateErr) {
        console.warn("Failed to auto-update yt-dlp, will attempt to use existing binary:", updateErr.message);
        if (!fs.existsSync(ytDlpPath)) {
            throw new Error(`yt-dlp is missing and could not be downloaded: ${updateErr.message}`);
        }
    }
}
...
async function getYouTubeTitle(url) {
    try {
        ...
        const ytDlp = new YTDlpWrap(fs.existsSync(ytDlpPath) ? ytDlpPath : undefined);
        ...
        await new Promise((res, rej) => {
            const ytDlpEmitter = ytDlp.exec(args);
            ...
            ytDlpEmitter.on('close', (code) => {
                if (code === 0) res();
                else rej(new Error(`yt-dlp metadata fetch exited with code ${code}`));
            });

            ytDlpEmitter.on('error', (err) => {
                rej(err);
            });
        });

        const info = JSON.parse(stdoutData.trim());
        const title = info.title || `YouTube Video ${await getYouTubeId(url)}`;
        ...
        console.error('Error fetching title via yt-dlp:', error.message);
...
async function getPlaylistInfo(url) {
    try {
        ...
        await new Promise((res, rej) => {
            ...
            ytDlpEmitter.on('close', (code) => {
                if (code === 0) res();
                else rej(new Error(`yt-dlp playlist fetch exited with code ${code}`));
            });
        ...
        for (const line of lines) {
            try {
                ...
                const title = parsed.title || `YouTube Video ${videoId}`;
                const thumbnail = parsed.thumbnail || (parsed.thumbnails && parsed.thumbnails.length > 0 ? parsed.thumbnails[parsed.thumbnails.length - 1].url : null);
                if (videoId) {
                    videos.push({
                        videoId,
                        title,
                ...
                console.warn("Failed to parse playlist item line:", err.message);
            }
        }

        return videos;
    } catch (error) {
        console.error('Error fetching playlist via yt-dlp:', error.message);
        throw error;
    }
}
...
async function downloadVideo(url, interaction) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // Ensure yt-dlp binary is up to date
            await ensureYtDlpUpdated();
            ...
            // Check for cookies.txt in project root or commands folder
            let cookiesPath = path.join(__dirname, '../../cookies.txt');
            if (!fs.existsSync(cookiesPath)) {
                cookiesPath = path.join(__dirname, 'cookies.txt');
            }
            ...
            if (fs.existsSync(cookiesPath)) {
                console.log(`Using cookies file from: ${cookiesPath}`);
                args.push('--cookies', cookiesPath);
            } else {
                console.log("No cookies.txt file found. Downloading without cookies.");
            }

            // Execute yt-dlp with real-time log piping
            await new Promise((res, rej) => {
                const ytDlpEmitter = ytDlp.exec(args);
    ...
    async execute(interaction) {
        ...
        if (!session) {
            ...
            if (voiceChannel.id !== session.voiceChannelId) {
                return interaction.editReply({ content: `I am already active in <#${session.voiceChannelId}>. Join me there or stop the bot first.` });
            }
        }
        ...
        session.collector.on('collect', async (i) => {
            if (i.member.voice.channelId !== session.voiceChannelId) {
                return i.reply({ content: "Join the voice channel to use controls!", flags: 64 });
            }
            ...
            try {
                if (i.isButton()) {
                    switch (i.customId) {
                        ...
                        case 'song_list':
                            ...
                            listToShow.forEach((title, idx) => {
                                const isPlaying = idx === session.queueIndex;
                                list += `${isPlaying ? '▶️' : `\`${idx}.\``} ${isPlaying ? `**${title}**` : title}\n`;
                            });
                            if (!list) list = "*The playlist is empty.*";
                        ...
                        case 'list_loop':
                            ...
                            await i.deferUpdate();
                            await session.updateEmbed();
                            // Feedback suppressed as requested
                        ...
                        case 'quit':
                            ...
                            await i.update({
                                components: [{
                                    type: ComponentType_Container,
                                    accent_color: 0x4F545C,
                                    components: [{
                                        type: ComponentType_TextDisplay,
                                        content: `### 👋 Session Ended\n*Thanks for listening! Use \`/playyt\` to start a new session.*`
                                    }]
                                }],
                                flags: MessageFlags_ComponentsV2
                            });
                    ...
                    if (i.customId === 'select_remove_song') {
                        ...
                        if (!isNaN(removeIndex) && session.songs.has(removeIndex)) {
                            ...
                            await i.followUp({ content: `Removed **${removedTitle}** from the playlist.`, flags: 64 });
                        } else {
                            await i.reply({ content: "Invalid selection.", flags: 64 });
                        }
                        ...
                        if (!isNaN(targetIndex) && session.songs.has(sourceIndex)) {
                            ...
                            await i.deferUpdate();
                            await session.updateEmbed();
                            await i.followUp({ content: `Moved **${titleToMove}** to position ${targetIndex}.`, flags: 64 });

                        } else {
                            await i.reply({ content: "Invalid move operation.", flags: 64 });
                        }
                ...
                if (!i.replied && !i.deferred) i.reply({ content: "Failed to process button.", flags: 64 });
            }
        });
                ...
                if (sessions.has(interaction.guildId)) {
                    session.message.edit({
                        components: [{
                            ...
                            components: [{
                                type: ComponentType_TextDisplay,
                                content: `### ⏰ Session Expired\n*Disconnected due to inactivity. Use \`/playyt\` to start again.*`
                            }]
                        }],
                        flags: MessageFlags_ComponentsV2
                    }).catch(() => { });
...
const attachModalListener = (client) => {
    ...
    client.on('interactionCreate', async interaction => {
        ...
        if (!session) {
            return interaction.reply({ content: "No active music session found.", flags: 64 });
        }
        ...
        try {
            ...
            // Detect common errors and give a clean message
            let userMsg = '❌ Failed to add song.';
            const raw = (error.message || '');
            if (raw.includes('Sign in to confirm') || raw.includes('not a bot')) {
                userMsg = '❌ YouTube blocked the download (bot detection). Make sure Chrome is open and logged in to YouTube, then try again.';
            } else if (raw.includes('Video unavailable') || raw.includes('410')) {
                userMsg = '❌ This video is unavailable or has been removed.';
            } else if (raw.includes('Private video')) {
                userMsg = '❌ This video is private.';
            } else {
                // Truncate raw error to fit Discord limit
                const truncated = raw.length > 1500 ? raw.substring(0, 1500) + '...' : raw;
                userMsg = `❌ Failed to add song:\n\`\`\`${truncated}\`\`\``;
            }
...
// Hook removed as we moved attachModalListener into execute()
